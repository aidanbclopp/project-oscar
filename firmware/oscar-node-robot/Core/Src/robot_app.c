...
#define IMU_STARTUP_GYRO_TIMEOUT_MS 3000U
...
#define MAIN_IMU_POLL_MS             500U
#define MAIN_ENC_LED_TOGGLE_TICKS    1500U

#define USER_ROUTINE_MOVE_INCHES     24.0f
#define USER_ROUTINE_TURN_DEG        180.0f

#define RADIO_EDGE_QUEUE_SIZE        512U
#define RADIO_EDGE_MIN_DELTA_US      140U
#define RADIO_EDGE_TICK_EVERY        32U
...
static RobotControlStatus_t App_RequestMove(float inches)
{
  if (imu_ready) {
    return Robot_MotionRunner_RequestMoveHeadingHold(
        &motor_l, &motor_r, &bno055, &heading_cfg, RADIO_MACRO_SPEED, inches);
  }
  return Robot_MotionRunner_RequestMoveEnc(&motor_l, &motor_r, RADIO_MACRO_SPEED, inches);
}
...
static RobotControlStatus_t App_RequestTurn(float degrees)
{
  if (imu_ready) {
    return Robot_MotionRunner_RequestRotateDeltaHeading(
        &motor_l, &motor_r, &bno055, &heading_cfg, RADIO_MACRO_SPEED, degrees);
  }
  return Robot_MotionRunner_RequestRotateEnc(&motor_l, &motor_r, RADIO_MACRO_SPEED, degrees);
}
...
static void App_UpdateEncoderStatusLed(void)
{
  ...
  while (tick_accum >= MAIN_ENC_LED_TOGGLE_TICKS) {
    HAL_GPIO_TogglePin(ENC_STATUS_GPIO_Port, ENC_STATUS_Pin);
    tick_accum -= MAIN_ENC_LED_TOGGLE_TICKS;
  }
}
...
static uint8_t App_RunImuStartupCalibration(void)
{
  ...
  while (1) {
    ...
    if (elapsed_ms >= IMU_STARTUP_GYRO_TIMEOUT_MS) {
      break;
    }
    ...
    if (gyro_calib >= IMU_CALIB_MIN_GYRO) {
      imu_ready = 1U;
...
static void App_ExecuteRadioDiscrete(CC1101_Command_t cmd)
{
  ...
  switch (cmd) {
    case CC1101_CMD_FORWARD:
      st = App_RequestMove(RADIO_MACRO_MOVE_INCHES);
      break;
    case CC1101_CMD_BACKWARD:
      st = App_RequestMove(-RADIO_MACRO_MOVE_INCHES);
      break;
    case CC1101_CMD_LEFT:
      st = App_RequestTurn(RADIO_MACRO_TURN_DEG);
      break;
    case CC1101_CMD_RIGHT:
      st = App_RequestTurn(-RADIO_MACRO_TURN_DEG);
      break;
    case CC1101_CMD_CENTER:
      st = App_RequestTurn(-360.0f);
      break;
    default:
      break;
  }
...
static void App_LogRadioOverflow(void)
{
  ...
  if ((radio_overflow_count == 0U) ||
      ((now_ms - radio_last_overflow_log_ms) < RADIO_OVERFLOW_LOG_PERIOD_MS)) {
    return;
  }
...
static void App_ProcessRadioRx(void)
{
  ...
  while (App_PopRadioEdge(&edge) != 0U) {
    ...
    edges_this_call++;
    if ((edges_this_call % RADIO_EDGE_TICK_EVERY) == 0U) {
...
{
  if (App_IsPeriodElapsed(now_ms, &periodic_state.imu_poll_ms, MAIN_IMU_POLL_MS)) {
    App_RefreshImuReadyFromSensor(0U);
  }

  if (App_IsPeriodElapsed(now_ms, &periodic_state.led_ms, MAIN_LED_BLINK_PERIOD_MS)) {
    HAL_GPIO_TogglePin(LED_GPIO_Port, LED_Pin);
  }

  if (App_IsPeriodElapsed(now_ms, &periodic_state.view_ms, MAIN_VIEW_REFRESH_MS)) {
...
static void App_UpdateUserRoutine(void)
{
  ...
  switch (user_routine_step) {
    ...
    case USER_ROUTINE_STEP_3:
      (void)App_RequestMove(USER_ROUTINE_MOVE_INCHES);
      break;
    case USER_ROUTINE_STEP_2:
      (void)App_RequestTurn(-USER_ROUTINE_TURN_DEG);
      break;
